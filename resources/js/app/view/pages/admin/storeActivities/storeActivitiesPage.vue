<template>
  <div>
    <div class="mt-3">
      ...
      <span>/</span>
      <label active>{{ $t("StoreActivities") }}</label>
    </div>
    <h4 class="mt-3">{{ $t("StoreActivities") }}</h4>
...
export default {
  ...
  data() {
    return {
      ...
      sortDirection: "asc",
      filter: null,
      filterOn: ["name", "groupName", "status"],
      addEditObj: {
        id: 0,
        group_id: "",
        name: "",
  ...
  methods: {
    ...
    deleteRow(idVal) {
      this.$confirm(
        ...
        this.$store
          .dispatch("activities/removeData", idVal)
          .then((_) => {
    ...
    initData(filters) {
      this.$store
        .dispatch("activities/getData", filters)
        .then((res) => {
    ...
    updateStatus(data) {
      ...
      this.$store
        .dispatch("activities/updateStatus", JSON.stringify(sendData))
        .then(() => {
          this.$notify.success({
            duration: 3000,
            message: this.$t("UpdatedSuccessfully"),
  ...
  created() {
    this.$store
      .dispatch("activities/listGroups")
      .then(res => {
