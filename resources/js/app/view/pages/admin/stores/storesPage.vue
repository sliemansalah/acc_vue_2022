<template>
    ...
    <div class="mt-3">
        ...
        <span>/</span>
        <label active>{{$t('Stores')}}</label>
    </div>
        <h4 class="mt-3">{{$t('Stores')}}</h4>
        ...
        methods: {
            ...
            deleteRow(idVal){
                this.$confirm(this.$t('DeleteConfirmMessage'), this.$t('DeleteConfirmTitle'), {
                    ...
                    this.$store.dispatch("stores/removeData", idVal)
                    .then(_ => {
            ...
            if (event.target.files.length) {
                ...
                this.$store
                .dispatch("stores/updateData", this.addEditObj)
                .then(_ => {
                    this.$notify.success({
                        duration: 3000,
            ...
            this.$store
                .dispatch("stores/updateStatus", JSON.stringify(sendData))
                .then(() => {
                let params = {
                    page: parseInt(this.pagination.currentPage),
                };
                    this.$notify.success({
                        duration: 3000,
                        message: this.$t("UpdatedSuccessfully"),
